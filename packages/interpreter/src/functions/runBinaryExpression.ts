...
export async function runBinaryExpression(
    ...
    switch (expr.operator) {
        case '+':
            ...
            if (isMeasurement(left) && isMeasurement(right)) {
                if (left.unit !== right.unit) {
                    throw new AstNodeError(expr, `Unit Mismatch: Cannot add measurements with different units (${left.unit} and ${right.unit})`);
                }
            ...
            throw new AstNodeError(expr, `Type Error: Cannot apply operator '+' to types ${typeof left} and ${typeof right}`);
        case '-':
            if (typeof left !== 'number' || typeof right !== 'number') throw new AstNodeError(expr, "Type Error: '-' requires number operands");
            return left - right;
        case '*':
            if (typeof left !== 'number' || typeof right !== 'number') throw new AstNodeError(expr, "Type Error: '*' requires number operands");
            return left * right;
        ...
        case '>=':
            ...
            return left >= right;
        case 'and':
            if (typeof left !== 'boolean' || typeof right !== 'boolean') throw new AstNodeError(expr, "Type Error: 'and' requires boolean operands");
            return left && right;
        case 'or':
        ...
        case '=':
            ...
            throw new AstNodeError(expr, "Assignment '=' should be handled in runExpression");
        default:
