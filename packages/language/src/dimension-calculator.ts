...
import { isUnitDeclaration, isUnitOperation, isUnitTerm, UnitDeclaration, UnitExpression } from './generated/ast.js';
...
export class DimensionCalculator {
    ...
    private computeExpression(expr: UnitExpression): DimensionVector {
        ...
        else if (isUnitTerm(expr)) {
            // Case: Reference to another unit (e.g., "meter")
            if (expr.ref && expr.ref.ref) {
                const baseVec = this.computeUnitDeclaration(expr.ref.ref);
                const power = expr.power ?? 1; // Default power is 1
                return DimensionCalculator.scaleVector(baseVec, power);
            }
        ...
        // If expr is neither (e.g. handled via parentheses transparency), 
        // Langium might pass the inner node. But TS types might enforce UnitExpression = Term | Op.
        // We covered both.

        return new Map<string, number>(); // Fallback (Dimensionless)
    }

    // --- Vector Math Helpers ---

    public static addVectors(v1: DimensionVector, v2: DimensionVector): DimensionVector {
    ...
    public static subtractVectors(v1: DimensionVector, v2: DimensionVector): DimensionVector {
        const result = new Map(v1);
        for (const [dim, exp] of v2) {
            result.set(dim, (result.get(dim) || 0) - exp);
        }
    ...
    public static scaleVector(v: DimensionVector, factor: number): DimensionVector {
