...
import { assertUnreachable, CreateParameterDetails, FunctionType, InferenceRuleNotApplicable, InferOperatorWithMultipleOperands, InferOperatorWithSingleOperand, NO_PARAMETER_NAME, TypeInitializer, TypirServices, ValidationProblemAcceptor } from "typir";
import { LangiumTypeSystemDefinition, TypirLangiumServices } from "typir-langium";
import { BinaryExpression, BooleanLiteral, CallExpression, ConstantDeclaration, ELangAstType, Expression, FormulaDeclaration, IfStatement, isConstantDeclaration, isFormulaDeclaration, isModelDeclaration, isMutableDeclaration, isNamedElement, isParameterDeclaration, MatchStatement, ModelDeclaration, MutableDeclaration, NumberLiteral, ReturnStatement, StringLiteral, TypeReference } from "./generated/ast.js";
...
export class ELangTypeSystem implements LangiumTypeSystemDefinition<ELangAstType> {
    onInitialize(typir: TypirLangiumServices<ELangAstType>): void {
        ...
        const prefixUnaryInferenceRule: InferOperatorWithSingleOperand<AstNode, Expression> = {
            languageKey: Expression,
            matching: (node: Expression, name: string) => node.prefixOperator === name,
            operand: (node: Expression, _name: string) => node,
            validateArgumentsOfCalls: true,
        };

        const postfixUnaryInferenceRule: InferOperatorWithSingleOperand<AstNode, Expression> = {
            languageKey: Expression,
            matching: (node: Expression, name: string) => node.postfixOperator === name,
            operand: (node: Expression, _name: string) => node,
            validateArgumentsOfCalls: true,
        };

        typir.factory.Operators.createUnary({ name: '!', signature: { operand: typeBool, return: typeBool } }).inferenceRule(prefixUnaryInferenceRule).finish();

        typir.factory.Operators.createUnary({ name: 'not', signature: { operand: typeBool, return: typeBool } }).inferenceRule(prefixUnaryInferenceRule).finish();

        typir.factory.Operators.createUnary({ name: '-', signature: { operand: typeNumber, return: typeNumber } }).inferenceRule(prefixUnaryInferenceRule).finish();

        typir.factory.Operators.createUnary({ name: '--', signature: { operand: typeNumber, return: typeNumber } }).inferenceRule(postfixUnaryInferenceRule).finish();
            ...
            typir.factory.Operators.createBinary({ name: operator, signature: { left: typeAny, right: typeAny, return: typeBool } })
                .inferenceRule({
                    ...
                    validation: (node, _operatorName, _operatorType, accept, typir) => typir.validation.Constraints.ensureNodeIsEquals(node.left, node.right, accept, (actual, expected) => ({
                        message: `This comparison will always return '${node.operator === '==' || node.operator === 'equals' ? 'false' : 'true'}' as '${node.left.$cstNode?.text}' and '${node.right.$cstNode?.text}' have the different types '${actual.name}' and '${expected.name}'.`,
                        languageNode: node,
        ...
        typir.factory.Functions.createUniqueFunctionValidation({ registration: { languageKey: FormulaDeclaration } });

        const uniqueModelValidator = typir.factory.Classes.createUniqueClassValidation({ registration: 'MYSELF' });
        ...
        typir.validation.Collector.addValidationRule(uniqueModelValidator, { languageKey: ModelDeclaration });

        typir.factory.Classes.createNoSuperClassCyclesValidation({ registration: { languageKey: ModelDeclaration } });
    ...
    protected createFormulaDetails(node: FormulaDeclaration, typir: TypirLangiumServices<ELangAstType>): TypeInitializer<FunctionType, AstNode> {
        ...
        if (isFormulaDeclaration(node)) {
            config.inferenceRuleForCalls({
                ...
                matching: (languageNode: CallExpression) => isFormulaDeclaration(languageNode.callee)
                    && languageNode.callee && languageNode.callee === node,
    ...
    protected validateReturnStatement(node: ReturnStatement, accept: ValidationProblemAcceptor<AstNode>, typir: TypirServices<AstNode>): void {
        const callableDeclaration: FormulaDeclaration | undefined = AstUtils.getContainerOfType(node, node => isFormulaDeclaration(node));
        ...
        } else if (callableDeclaration && callableDeclaration.returnType && !node.value) {
            typir.validation.Constraints.ensureNodeIsAssignable(node, callableDeclaration.returnType, accept, (actual, expected) => ({
    ...
    protected validateVariableDeclaration(node: ConstantDeclaration | MutableDeclaration, accept: ValidationProblemAcceptor<AstNode>, typir: TypirServices<AstNode>): void {
    ...
    protected validateIfConditions(node: IfStatement, accept: ValidationProblemAcceptor<AstNode>, typir: TypirServices<AstNode>): void {
        const typeBool = typir.factory.Primitives.get({ primitiveName: 'boolean' })!;

        for (const condition of node.conditions) {
            typir.validation.Constraints.ensureNodeIsAssignable(condition, typeBool, accept,
                () => ({ message: "Conditions need to evaluate to 'boolean'.", languageProperty: 'condition' }));
        }
    }

    protected validateMatchCondition(node: MatchStatement, accept: ValidationProblemAcceptor<AstNode>, typir: TypirServices<AstNode>): void {
        const typeBool = typir.factory.Primitives.get({ primitiveName: 'boolean' })!;
