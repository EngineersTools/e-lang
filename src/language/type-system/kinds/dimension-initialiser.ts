import { TypeInitializer, TypeStateListener } from "typir";
import { DimensionType } from "./dimension-type.js";
import { CreateDimensionTypeDetails, DimensionKind } from "./dimension-kind.js";
import { ELangTypirServices } from "../e-lang-type-services.js";

export class DimensionTypeInitializer<LanguageType> extends TypeInitializer<DimensionType, LanguageType> implements TypeStateListener {
    ...
    protected initialDimensionType: DimensionType;

    constructor(services: ELangTypirServices<LanguageType>, kind: DimensionKind<LanguageType>, typeDetails: CreateDimensionTypeDetails<LanguageType>) {
        super(services);
        this.typeDetails = typeDetails;
        this.kind = kind;

        // create the class type
        this.initialDimensionType = new DimensionType(kind as DimensionKind<unknown>, typeDetails as CreateDimensionTypeDetails<unknown>);
        if (kind.options.typing === 'Structural') {
            ...
            this.services.infrastructure.Graph.addNode(this.initialDimensionType, kind.calculateIdentifierWithDimensionNameOnly(typeDetails));
        }

        this.createInferenceAndValidationRules(this.typeDetails, this.initialDimensionType);
        // register all the inference rules already now to enable early type inference for this Dimension type ('undefined', since its Identifier is still missing)
        ...
        this.initialDimensionType.addListener(this, true); // trigger directly, if some initialization states are already reached!
    }

    onSwitchedToIdentifiable(classType: Type): void {
        /* Important explanations:
         * - This logic here (and 'producedType(...)') ensures, that the same DimensionType is not registered twice in the type graph.
         ...
         * - Accordingly, 'classType' and 'readyDimensionType' might have different values!
         */
        assertTypirType(classType, isDimensionType);
        const readyDimensionType = this.producedType(classType);

        // remove/invalidate the duplicated and skipped class type now
        if (readyDimensionType !== classType) {
            ...
            classType.removeListener(this); // since this DimensionTypeInitializer initialized the invalid type, there is nothing to do anymore here!

            if (this.kind.options.typing === 'Structural') {
                // replace the type in the type graph
                const nameBasedIdentifier = this.kind.calculateIdentifierWithDimensionNameOnly(this.typeDetails);
                this.services.infrastructure.Graph.removeNode(classType, nameBasedIdentifier);
                this.services.infrastructure.Graph.addNode(readyDimensionType, nameBasedIdentifier);
            }
            ...
            this.createInferenceAndValidationRules(this.typeDetails, readyDimensionType);
            // add the new rules
            this.inferenceRules.forEach(rule => this.services.Inference.addInferenceRule(rule.rule, optionsBoundToType(rule.options, readyDimensionType)));
            this.validationRules.forEach(rule => this.services.validation.Collector.addValidationRule(rule.rule, optionsBoundToType(rule.options, readyDimensionType)));
        } else {
    ...
    onSwitchedToCompleted(classType: Type): void {
        ...
        if (this.typeDetails.inferenceRulesForDimensionDeclaration === null) {
            // check for cycles in sub-type-relationships of classes
            if ((classType as DimensionType).hasSubSuperDimensionCycles()) {
    ...
    onSwitchedToInvalid(_previousDimensionType: Type): void {
        // nothing specific needs to be done for Dimensiones here, since the base implementation takes already care about all relevant stuff
    }

    override getTypeInitial(): DimensionType {
        return this.initialDimensionType;
    }

    protected createInferenceAndValidationRules(typeDetails: CreateDimensionTypeDetails<LanguageType>, classType: DimensionType): void {
        ...
        for (const inferenceRulesForDimensionDeclaration of typeDetails.inferenceRulesForDimensionDeclaration) {
            this.inferenceRules.push(bindInferCurrentTypeRule<DimensionType, LanguageType>(inferenceRulesForDimensionDeclaration, classType));
            // TODO check values for fields for structual typing!
            const validationRule = bindValidateCurrentTypeRule<DimensionType, LanguageType>(inferenceRulesForDimensionDeclaration, classType);
            if (validationRule) {
                this.validationRules.push(validationRule);
            }
        }
        for (const inferenceRulesForDimensionLiterals of typeDetails.inferenceRulesForDimensionLiterals) {
            this.inferenceRules.push(this.createInferenceRuleForLiteral(inferenceRulesForDimensionLiterals, classType));
            const validationRule = this.createValidationRuleForLiteral(inferenceRulesForDimensionLiterals, classType);
            if (validationRule) {
                this.validationRules.push(validationRule);
            }
        }
    ...
    protected createInferenceRuleForLiteral<T extends LanguageType>(rule: InferDimensionLiteral<LanguageType, T>, classType: DimensionType): InferenceRuleWithOptions<LanguageType, T> {
    ...
    protected createValidationRuleForLiteral<T extends LanguageType>(rule: InferDimensionLiteral<LanguageType, T>, classType: DimensionType): ValidationRuleWithOptions<LanguageType, T> | undefined {
